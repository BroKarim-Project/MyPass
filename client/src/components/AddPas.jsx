...
export default function AddPas({ onClose, fetchPasswords }) {
  ...
  const addPassword = async () => {
    ...
    if (!token || !googleUserId) return;

    try {
      const response = await fetch('http://localhost:3001/passwords/add', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          password: password,
          title: title,
          google_user_id: googleUserId, // Kirim google_user_id yang benar
        }),
      });
      ...
      await fetchPasswords(); // Fetch updated data
      closeModal();
    } catch (error) {
